        ...
        <div class="col-lg-8 mr-auto">
            <div class="breadcrumb-content">
                ...
                <p class="pt-2 pb-1">Created by <a href="teacher-detail.html"
                        class="text-color hover-underline">{{ $course->instuctor->name }}</a></p>
            ...
            <div class="col-lg-8 pb-5">
                <div class="course-details-content-wrap pt-90px">
                    ...
                    <div class="course-overview-card">
                        ...
                        @php
                        $sections =
                        App\Models\CourseSection::where('course_id',$course->id)->orderBy('id','ASC')->get();
                        @endphp
                        ...
                        <div class="curriculum-content">
                            <div id="accordion" class="generic-accordion">
                                ...
                                @php
                                $lecture = App\Models\CourseLecture::where('secation_id',$section->id)->get();
                                @endphp
                            ...
                            <div class="media media-card">
                                <div class="instructor-img">
                                    <a href="teacher-detail.html" class="media-img d-block">
                                        <img class="lazy"
                                            src="{{ (!empty($course->instuctor->photo)) ? url('uploads/instuctor_img/'.$course->instuctor->photo) : url('uploads/no_image.jpg') }}"
                                            data-src="{{ (!empty($course->instuctor->photo)) ? url('uploads/instuctor_img/'.$course->instuctor->photo) : url('uploads/no_image.jpg') }}"
                                            alt="Avatar image">
                                    </a>
                                    <ul class="generic-list-item pt-3">
                                        ...
                                        <li><i class="la la-play-circle-o mr-2 text-color-3"></i> {{ count($ins_courses) }} Courses</li>
                                        <li><a href="teacher-detail.html">View all Courses</a></li>
                                    </ul>
                                </div><!-- end instructor-img -->
                                <div class="media-body">
                                    <h5><a href="teacher-detail.html">{{ $course->instuctor->name }}</a></h5>
                                    <span class="d-block lh-18 pt-2 pb-3">Joined
                                        {{ Carbon\Carbon::parse($course->instuctor->created_at)->diffForHumans()  }}
                                    </span>
                                    <p class="text-black lh-18 pb-3">{{ $course->instuctor->email }}</p>
                                    <p class="d-block p-2 lh-18">{{ $course->instuctor->description }}</p>

                                    <div class="collapse" id="collapseMoreTwo">
                                        <p class="pb-3">{{ $course->instuctor->long_description }}</p>
                                    </div>
                        ...
                        <div class="card-body">
                            <div class="preview-course-video">
                                <a href="javascript:void(0)" data-toggle="modal" data-target="#previewModal">
                                    <img src="{{ asset('uploads/course/course_image') }}/{{  $course->course_image }}" data-src="{{ asset('uploads/course/course_image') }}/{{  $course->course_image }}" alt="course-img"
                    ...
                    <div class="card card-item">
                        <div class="card-body">
                            ...
                            <ul class="generic-list-item generic-list-item-flash">
                                ...
                                <li class="d-flex align-items-center justify-content-between"><span><i
                                            class="la la-eye mr-2 text-color"></i>Preview Lessons</span> {{ count($sections) }}</li>
                                <li class="d-flex align-items-center justify-content-between"><span><i
                        ...
                        <div class="card-body">
                            ...
                            <div class="media media-card border-bottom border-bottom-gray pb-4 mb-4">
                                <a href="{{ url('course/details/'.$item->id.'/'.$item->course_name_slug) }}" class="media-img">
                                    <img class="mr-3 lazy" src="{{ asset('uploads/course/course_image') }} / {{ $item->course_image }}"
                                        data-src="{{ asset('uploads/course/course_image') }}/{{ $item->course_image }}" alt="Related course image">
                                        {{-- {{ url('course/details/'.$ins_course->id.'/'.$ins_course->course_name_slug) }} --}}
                                </a>
                                <div class="media-body">
                                    <h5 class="fs-15"><a href="{{ url('course/details/'.$item->id.'/'.$item->course_name_slug) }}">{{ $item->course_name }}</a></h5>
                                    <span class="d-block lh-18 py-1 fs-14">{{ $item->instuctor->name }}</span>

                            @if ($item->discount_price == NULL )

                            <p class="text-black font-weight-semi-bold lh-18 fs-15">${{ $item->selling_price }} </p>


                            @else

                            <p class="text-black font-weight-semi-bold lh-18 fs-15">${{ $item->discount_price }}
                                <span class="before-price fs-14"> ${{ $item->selling_price }}</span>
                            </p>
    ...
    <div class="container">
        <div class="related-course-wrap">
            <h3 class="fs-28 font-weight-semi-bold pb-35px">More Courses by <a href="teacher-detail.html"
                    class="text-color hover-underline">{{ $course->instuctor->name }}</a></h3>
            <div class="view-more-carousel-2 owl-action-styled">

                @foreach ($ins_courses as $ins_course)
                <div class="card card-item">
                    <div class="card-image">
                        <a href="{{ url('course/details/'.$ins_course->id.'/'.$ins_course->course_name_slug) }}" class="d-block">
                            <img class="card-img-top" src="{{ asset('uploads/course/course_image') }}/{{  $ins_course->course_image }}" alt="Card image cap">
                        </a>
                        <div class="course-badge-labels">

                            {{-- Discount Calculation --}}
                            @php
                            $amount = $ins_course->selling_price - $ins_course->discount_price;
                            $discount = ($amount / $ins_course->selling_price) * 100;
                            @endphp
                            {{-- Discount Calculation --}}
                            ...
                            @if (!$ins_course->discount_price == NULL)
                    ...
                    <div class="card-body">
                        <h6 class="ribbon ribbon-blue-bg fs-14 mb-3">{{ $ins_course->label }}</h6>
                        <h5 class="card-title"><a href="{{ url('course/details/'.$ins_course->id.'/'.$ins_course->course_name_slug) }}">{{ $ins_course->course_name }}</a></h5>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <p class="card-price text-black font-weight-bold">${{ $ins_course->selling_price }} </p>


                            @else

                            <p class="card-price text-black font-weight-bold">${{ $ins_course->discount_price }}
                                <span class="before-price font-weight-medium"> ${{ $ins_course->selling_price }}</span>
                            </p>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <video controls crossorigin playsinline
                    poster="{{ asset('uploads/course/course_image') }}/{{  $course->course_image }}" id="player">
                    <!-- Video files -->
                    <source src="{{ asset($course->course_video) }}"
                        type="video/mp4" />
                </video>
            </div><!-- end modal-body -->
        </div><!-- end modal-content -->
